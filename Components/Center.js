  ...
  return (
    <div className="flex-grow text-white overflow-y-scroll h-screen scrollbar-hide">
      <header className="absolute top-5 right-8">  
          <div className="flex w-max items-center bg-black space-x-3 opacity-90 hover:opacity-80 cursor-pointer rounded-full p-1 pr-2" onClick={() => signOut()}>
      ...
      <section className={`flex items-end w-full h-80 space-x-7 bg-gradient-to-b to-black ${color} text-white p-8`}>
        <img src={playlist?.images?.[0]?.url} alt="" className="h-44 w-44 rounded-md shadow-2xl"/>
        <div>
          <h1>PLAYLIST</h1>
